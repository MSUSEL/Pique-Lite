...
// ../../node_modules/.pnpm/tsup@8.0.2_postcss@8.4.49_ts-node@10.9.2_@types+node@20.12.11_typescript@5.4.5__typescript@5.4.5/node_modules/tsup/assets/cjs_shims.js
...
var machineId = `mil-${(0, import_node_crypto.randomUUID)()}`;
...
init_cjs_shims();
var ft11 = __toESM(require("faster-babel-types"));

// compiler/src/core/capture/metadata.ts
init_cjs_shims();
var ft7 = __toESM(require("faster-babel-types"));
...
init_cjs_shims();
var ft = __toESM(require("faster-babel-types"));
...
init_cjs_shims();
var ft2 = __toESM(require("faster-babel-types"));
...
init_cjs_shims();
var ft4 = __toESM(require("faster-babel-types"));

// compiler/src/core/utils/generate-unique-name.ts
init_cjs_shims();
var ft3 = __toESM(require("faster-babel-types"));
...
init_cjs_shims();
var ft5 = __toESM(require("faster-babel-types"));
...
init_cjs_shims();
var ft6 = __toESM(require("faster-babel-types"));
...
// compiler/src/core/capture/transform.ts
var ft9 = __toESM(require("faster-babel-types"));

// compiler/src/core/utils/get-definition.ts
init_cjs_shims();
var ft8 = __toESM(require("faster-babel-types"));
...
var ft10 = __toESM(require("faster-babel-types"));
...
ingestServer.use((0, import_cors.cors)()).use(decompress()).post("/ingest", async (c) => {
  ...
  if (process.env.NODE_ENV !== "production") {
    if (!process.env.IS_APM) {
      if (body.compilerInstanceId !== (0, import_ohash.hash)(process.cwd())) {
...
var import_ohash2 = require("ohash");
var pendingWork = { current: [] };
var createWsConnection = async (host, port, query) => {
  return new Promise((res, rej) => {
    const socket = (0, import_socket2.io)(`http://${host}:${port}`, {
      path: "/ws",
      query: {
        cwd: process.cwd(),
        ...query
...
var injectWebSocket = (server2) => {
  const ioServer = new import_socket.Server(server2, {
    path: "/ws",
    serveClient: false,
    cors: {
      origin: "*",
  ...
  ioServer.use(async (socket, next3) => {
    await Promise.all(pendingWork.current);
    socket.handshake.query.serverid = (0, import_ohash2.hash)(process.cwd());
  ...
  ioServer.on("connection", async (runtimeSocket) => {
    ...
    runtimeSocket.emit("serverInfo", (0, import_ohash2.hash)(process.cwd()));
...
var getNextCacheDir = (cwd3) => {
  return import_node_path2.default.join(cwd3, ".next", "cache", "webpack", "client-development");
};
var getWebpackCacheDir = (cwd3) => {
  return import_node_path2.default.join(cwd3, "node_modules", ".cache");
};
var getViteCacheDir = (cwd3) => {
  return import_node_path2.default.join(cwd3, "node_modules", ".vite");
};
var getLastModifiedTimeCache = (cwd3) => {
  const nextCacheDir = import_node_path2.default.join(getNextCacheDir(cwd3), "index.pack.gz");
  if (import_node_fs2.default.existsSync(nextCacheDir)) {
    return import_node_fs2.default.statSync(nextCacheDir).mtime.getTime();
  }
  return null;
};
var bustBuildCache = (cwd3) => {
  ...
  import_node_fs2.default.rmSync(nextCacheDir, { recursive: true, force: true });
  import_node_fs2.default.rmSync(webpackCacheDir, { recursive: true, force: true });
  import_node_fs2.default.rmSync(viteCacheDir, { recursive: true, force: true });
};
...
var buildCacheHandler = (cwd3, file) => {
  return {
    write: (data) => {
      const baseDir = import_node_path3.default.join(cwd3, ".million");
      try {
        if (!import_node_fs3.default.existsSync(import_node_path3.default.join(baseDir))) {
          import_node_fs3.default.mkdirSync(import_node_path3.default.join(baseDir));
        }
        import_node_fs3.default.writeFileSync(import_node_path3.default.join(baseDir, file), JSON.stringify(data));
      } catch (err) {
    ...
    load: () => {
      const baseDir = import_node_path3.default.join(cwd3, ".million");
      try {
        if (!import_node_fs3.default.existsSync(import_node_path3.default.join(baseDir, file))) {
          return null;
        }
        return JSON.parse(
          import_node_fs3.default.readFileSync(import_node_path3.default.join(baseDir, file), "utf8")
        );
      } catch (err) {
...
var transform = async (instance, meta, options, code, id, map) => {
  ...
  if ((options == null ? void 0 : options.skipTransform) || store.unusedFiles.has(id))
    return null;
  const relativePath = import_node_path4.default.relative(cwd2, id);
...
function compareSemver2(v1, v2) {
  const cleanVersion = (v) => {
    ...
    const parts = v.split(/[-.]/);
    let i = 0;
    while (i < parts.length && !isNaN(Number(parts[i])))
      i++;
    const version = parts.slice(0, i).map(Number);
    const metadata = parts.slice(i).join("-");
    if (v.endsWith(".tgz")) {
  ...
  const v1Info = cleanVersion(v1);
  const v2Info = cleanVersion(v2);
  for (let i = 0; i < 3; i++) {
    const n1 = v1Info.version[i] || 0;
    const n2 = v2Info.version[i] || 0;
    if (n1 !== n2)
      return n1 < n2 ? -1 : 1;
  }
  if (v1Info.metadata === v2Info.metadata)
    return 0;
  if (!v1Info.metadata)
    return 1;
  if (!v2Info.metadata)
  ...
  const getWeight = (metadata) => {
    for (const [key, weight] of Object.entries(weights)) {
      if (metadata.toLowerCase().includes(key)) {
        return weight;
      }
    }
    return 0;
  };
  const w1 = getWeight(v1Info.metadata);
  const w2 = getWeight(v2Info.metadata);
  if (w1 !== w2)
    return w1 < w2 ? -1 : 1;
...
var findDir = (dir, name) => {
  let currDir = (0, import_node_path5.join)(dir, name);
  if ((0, import_node_fs4.existsSync)(currDir))
    return currDir;
  currDir = (0, import_node_path5.join)(dir, "src", name);
  if ((0, import_node_fs4.existsSync)(currDir))
    return currDir;
  return null;
};
var getNextVersion = () => {
  try {
    const packageJson2 = (0, import_node_fs4.readFileSync)(
      "./node_modules/next/package.json",
      // may be problematic if next is installed at root of a monorepo
      "utf-8"
    );
    const json = JSON.parse(packageJson2);
    const nextVersion = json["version"];
    return nextVersion;
  } catch {
    return void 0;
  }
...
var next = (options = {}) => {
  return (nextConfig = {}) => {
    ...
    if (process.env.TURBOPACK && options.turbo !== false) {
      const injectLoader = (glob) => {
        var _a, _b;
        const version = getNextVersion();
        if (version) {
          const unsupportedNextVersion = (
            // 15.0.0 === minimum supported version
            compareSemver2(version, "15.0.0") === -1
          );
          if (unsupportedNextVersion) {
            logError(
              "\n[Million.js] Error: Incompatible Next.js version detected when using Turbopack\n\n   Million.js plugin requires Next.js 15 or later when using Turbopack.\n   Current Next.js version: " + version + '\n\n   To fix this, either:\n   1. Upgrade to Next.js 15 or later\n      npm install next@latest\n   2. Remove --turbopack from your dev script\n      "dev": "next dev"\n\n   Learn more: https://million.dev/docs\n'
            );
            return nextConfig;
          }
        }
        ...
        rule.loaders.unshift("@million/lint/loader");
      };
      injectLoader("**/*.tsx");
      injectLoader("**/*.jsx");
      return nextConfig;
    ...
    return {
      ...nextConfig,
      // TODO: not compatible with Next.js 13
      reactProductionProfiling: process.env.NODE_ENV === "production" || process.env.NODE_ENV === "test" || process.env.NODE_ENV !== "development",
