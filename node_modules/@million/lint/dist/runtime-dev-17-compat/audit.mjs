...
var logAudit = () => {
  ...
  for (let i = 0, len = table.length; i < len; i++) {
    ...
    console.log(
      ...compileMarkdown(
        [
          `**${prettyMs(component.selfTime)}** _(\xD7${component.count})_ \xB7 \`${component.componentName}\``
        ],
        component.source
      )
    );
  ...
  console.log(
    `%cYou can potentially save: ${prettyMs(savings)} (${Math.round(improvement * 100)}% faster)`,
    "background: red; color: white; font-size: 20px; padding: 10px;"
  );
};
var interval;
