    ...
    <Dialog>
      ...
      <DialogPortal>
        ...
        >
          ...
          <div style={{ width: "20%", borderRight: "1px solid #ddd", paddingRight: "10px" }}>
            ...
            {Object.entries(projects).map(([uuid, project]) => (
              <div
                ...
                style={{
                  padding: "8px",
                  cursor: "pointer",
                  backgroundColor: uuid === currentProjectId ? "#e0e0e0" : "transparent",
                }}
              >
                {editProjectId === uuid ? (
                <input
            ...
            <Button variant="outline" size="4" onClick={handleNewProject} style={{ marginTop: "10px" }}>
              + Create New Project
            </Button>
          </div>
          ...
          <div style={{ width: "60%", paddingLeft: "10px", display: "flex", flexDirection: "column", alignItems: "center" }}>
            ...
            {files.length > 0 && allFilesVerified() && (
              <>
                <Button variant="solid" size="4" radius="large" style={{ marginTop: "20px" }} onClick={handleAddFiles}>
                  Add Files
                </Button>

                {existingFiles.length > 0 && (
                  <DialogClose asChild>
                    <Button variant="solid" size="4" radius="large" style={{ marginTop: "10px" }} onClick={handleContinue}>
                      Continue
                    </Button>
                  </DialogClose>
                )}
            ...
            {files.length > 0 && !files.every((file) => file.verified) && (
              <Callout.Root color="red" style={{ marginTop: "20px" }} size="2">
                <Callout.Icon><InfoCircledIcon /></Callout.Icon>
                <Callout.Text>Please delete unverified files to continue.</Callout.Text>
              </Callout.Root>
            )}
          </div>
        </DialogContent>
