...
## 4. Tried but did not work (Notes for future developers)
...
## 6. Resolved Design Notes
### 1\. Line chart brush mode and timestamp handling

PIQUE Lite is a fully client-side tool that accepts input JSON files generated by different pipelines and tools. As a result, uploaded version files may differ in whether and how temporal information (e.g., timestamps) is included.

#### Background

The line chart brush interaction (used to select a range of versions over time) implicitly assumes that each data point has a stable, sortable x-axis value. Earlier mock datasets satisfied this assumption by including consistent time information. However, newer input formats may:

- Omit explicit timestamps
- Encode dates as strings rather than numeric values
- Provide version ordering without a guaranteed temporal field

This mismatch caused brush interactions to fail even though tooltip and hover interactions continued to work.

#### Current solution

To make brush interactions robust across heterogeneous input formats, we normalized the x-axis used by line charts:

- Each chart data point is assigned a numeric `x` value (derived from `Date.getTime()` when a date is available).
- Human-readable date strings are preserved separately for axis labels and tooltips.
- Line charts and brush interactions now operate on the numeric `x` field rather than raw date objects or strings.

This approach ensures that brush and zoom interactions remain stable regardless of how dates are represented in the original input files.

#### Recommendation for future input generators

For tools or pipelines that generate PIQUE-compatible input files, we recommend including an explicit timestamp field for each version whenever possible.

Preferred format:
- A numeric Unix timestamp in milliseconds (e.g., `timestamp: 1704067200000`), or
- An ISO 8601 date-time string (e.g., `2024-12-31T00:00:00Z`)

If such a timestamp is provided, PIQUE Lite can directly map it to the numeric x-axis used by charts, enabling consistent brush and zoom behavior without additional adaptation.
...
- Local development: use the CLI commands in README for development/debugging.

### Branch roles used in this repo
- `refactor`: primary development branch for feature work.
- `demo`: demo-facing branch; occasionally force-aligned to `refactor` to avoid extra commits.
- `gh-pages`: hosting branch for the static site only (should remain minimal and clean).

### What we tried / changed / abandoned (Dec 2025)
- We iterated on multiple GitHub Pages deployment commits (SPA entry / index / fallback variations) to restore correct routing and asset loading.
- We attempted to revert `gh-pages` to a known-good commit, but the process was repeatedly blocked by local artifacts (notably `.DS_Store`) and accidental pollution from local build/install outputs.
- Decision: avoid long revert sequences on `gh-pages` when the working tree is polluted; prefer a clean re-clone + direct rollback to a known-good commit with a backup branch.

### Guardrails for future redeployments
- Do not run package installs or development workflows on `gh-pages` (avoid `node_modules` / local artifacts).
- Always create a backup branch before rewriting `gh-pages`.
- Prefer `reset --hard <known-good>` + `push --force-with-lease` for restoring the live site.
